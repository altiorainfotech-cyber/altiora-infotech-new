...
import { Compass, Palette, Video, Share2, Users, TrendingUp, RefreshCw } from "lucide-react";

const STAGES = [
  { id: "audit", name: "Social Audit", icon: Compass, desc: "Audience & competitor analysis" },
  { id: "pillars", name: "Content Pillars", icon: Palette, desc: "Brand voice & visual guidelines" },
  { id: "creation", name: "Short-Form Production", icon: Video, desc: "High-converting Reels & TikToks" },
  { id: "publishing", name: "Multi-Channel Distribution", icon: Share2, desc: "Automated schedule & cross-posting" },
  { id: "community", name: "Community Management", icon: Users, desc: "Active engagement & DM nurturing" },
  { id: "conversion", name: "Lead Attribution", icon: TrendingUp, desc: "Turning followers into customers" },
  { id: "scale", name: "Viral Velocity", icon: RefreshCw, desc: "Scaling winning creative formats" },
];

export function SocialMediaJourney() {
    ...
    <div ref={containerRef} className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-slate-900 via-blue-950 to-ink p-6 sm:p-8 text-white shadow-2xl overflow-hidden">
      ...
      <div className="relative z-10 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-6">
        <div>
          ...
          <h3 className="text-xl sm:text-2xl font-black tracking-tight text-white mt-1">Social Media Growth Architecture</h3>
        </div>
        <div className="flex items-center gap-2 rounded-full border border-blue-400/30 bg-blue-400/10 px-3.5 py-1 text-xs font-bold text-blue-300 backdrop-blur-md">
          ...
          <span>Viral Engagement Pipeline</span>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="my-3 flex items-center gap-3 lg:flex-col lg:items-start">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-blue-900 text-white shadow-md border border-white/20">
