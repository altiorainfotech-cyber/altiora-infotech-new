...
export default function AeoGeoHeroVisual() {
  ...
  useEffect(() => {
    ...
    const pointLightPurple = new THREE.PointLight(0x8b5cf6, 6, 25);
    pointLightPurple.position.set(6, 4, 6);
    ...
    // Central "AI KNOWLEDGE GRAPH" core node
    ...
    // Orbiting AI Model Nodes (ChatGPT, Perplexity, Claude, Gemini, SGE)
    const nodeTypes = [
      { name: "ChatGPT", color: 0x8b5cf6, radius: 2.8, speed: 0.5, size: 0.22 },
      { name: "Perplexity", color: 0xec4899, radius: 3.4, speed: -0.4, size: 0.18 },
      { name: "Claude", color: 0xd3ac3c, radius: 2.2, speed: 0.7, size: 0.24 },
      { name: "Gemini", color: 0x3b82f6, radius: 3.8, speed: -0.3, size: 0.25 },
      { name: "Overviews", color: 0x10b981, radius: 4.4, speed: 0.25, size: 0.28 },
    ];
    ...
    ringRadii.forEach((r, idx) => {
      ...
      const mat = new THREE.MeshStandardMaterial({
        color: idx % 2 === 0 ? 0x8b5cf6 : 0xd3ac3c,
        emissive: idx % 2 === 0 ? 0x7c3aed : 0xc9a227,
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const col = colorPick > 0.6 ? cPurple : colorPick > 0.3 ? cPink : cGold;
