...
import { RefreshCw, ArrowRight, Bot, Sparkles, Cpu, Braces, Award, Activity } from "lucide-react";

const LOOP_STEPS = [
  { step: "AUDIT", icon: Bot, desc: "Crawl AI model citation share" },
  { step: "STRUCTURE", icon: Braces, desc: "Deploy JSON-LD schema & entity graphs" },
  { step: "OPTIMIZE", icon: Sparkles, desc: "Format answer-first conversational copy" },
  { step: "AUTHORITY", icon: Award, desc: "Strengthen E-E-A-T & digital PR citations" },
  { step: "MONITOR", icon: Activity, desc: "Track ChatGPT, Perplexity & Copilot" },
  { step: "SCALE", icon: Cpu, desc: "Expand generative AI search presence" },
];

export function AeoGeoOptimizationLoop() {
  return (
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-6 sm:p-10 shadow-xl overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-ink/8 pb-5 mb-8">
        ...
        <div className="flex items-center gap-2 text-xs font-mono font-bold text-blue-900 bg-blue-100/80 px-3.5 py-1.5 rounded-full border border-blue-200">
          ...
          <span>CONTINUOUS AI CITATION COMPOUNDING</span>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-ink/70 font-mono bg-surface p-3 rounded-xl border border-ink/5">
        ...
        <span className="text-blue-800">FEEDS CITATIONS BACK TO DOMAIN AUTHORITY &amp; TRUST</span>
