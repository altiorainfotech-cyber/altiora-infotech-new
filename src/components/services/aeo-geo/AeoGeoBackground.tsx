...
export default function AeoGeoBackground() {
  ...
  useEffect(() => {
    ...
    // Ecosystem group: a neural-network-like node cluster representing AI answer engines
    ...
    // Concentric "answer engine" rings
    ...
    // Node cluster with connecting edges — a small neural-net motif
    ...
    // Thin connecting edges between neighboring nodes
    const edgeMat = new THREE.LineBasicMaterial({ color: 0x8fb0e0, transparent: true, opacity: 0.25 });
    const edgeGeo = new THREE.BufferGeometry();
    const edgePositions = new Float32Array(nodeCount * 2 * 3);
      ...
      if (!prefersReducedMotion) {
        ...
        nodes.forEach((node, i) => {
          ...
          node.mesh.position.set(
            ...
            node.baseY + Math.sin(t * 0.4 + node.offset) * 0.3,
            Math.sin(angle) * node.radius
          );
          node.mesh.rotation.x += 0.01;
          node.mesh.rotation.y += 0.008;
          nodePositions[i].copy(node.mesh.position);
        });

        // Update connecting edges between each node and the next (ring topology)
