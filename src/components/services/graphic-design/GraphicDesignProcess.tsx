...
import { graphicDesignProcess, ProcessStep } from "@/data/graphicDesign";
import { Sparkles, RefreshCw } from "lucide-react";

export function GraphicDesignProcess() {
  ...
  return (
    <section ref={containerRef} className="relative overflow-hidden bg-transparent py-14 sm:py-24" aria-labelledby="graphic-process-heading">
      ...
      <Container>
        <div className="max-w-2xl">
          ...
          <Reveal delay={0.05}>
            <h2 id="graphic-process-heading" className="mt-3 text-3xl font-black tracking-tight text-ink sm:text-5xl leading-tight">
              Continuous Design Lifecycle
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-3 text-sm sm:text-base leading-relaxed text-muted font-medium">
              Watch your visual design system construct progressively as scroll triggers each stage of creative execution.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mt-14 rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-blue-950 to-ink p-6 sm:p-12 text-white shadow-2xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
            {graphicDesignProcess.map((step: ProcessStep, idx: number) => {
              const Icon = step.icon;
              const threshold = (idx / (graphicDesignProcess.length - 1)) * 0.8;
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-between border-t border-white/10 pt-6 text-xs font-mono font-bold text-white/70 gap-4">
            <div className="flex items-center gap-2">
              ...
              <span>STAGE 06 (DELIVER) FEEDS ASSET TOKENS BACK TO STAGE 01 (DISCOVERY)</span>
            </div>
            <div className="flex items-center gap-2 text-gold-400 bg-gold-400/10 px-3 py-1.5 rounded-full border border-gold-400/30">
              ...
              <span>INFINITE CREATIVE FLYWHEEL</span>
            </div>
          </div>
        </div>
      </Container>
