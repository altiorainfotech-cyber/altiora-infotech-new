...
import { whyChooseGraphicDesign, WhyChoosePoint } from "@/data/graphicDesign";
import { ShieldCheck } from "lucide-react";

export function GraphicDesignWhyChoose() {
  return (
    <section className="relative overflow-hidden bg-transparent py-14 sm:py-24" aria-labelledby="graphic-why-heading">
      <Container>
        <div className="max-w-2xl">
          ...
          <Reveal delay={0.05}>
            <h2 id="graphic-why-heading" className="mt-3 text-3xl font-black tracking-tight text-ink sm:text-5xl leading-tight">
              Why Brands Choose Altiora for Graphic Design
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-3 text-sm sm:text-base leading-relaxed text-muted font-medium">
              Strategic visual design engineered to build brand recall and conversion power.
            </p>
          </Reveal>
        </div>

        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {whyChooseGraphicDesign.map((item: WhyChoosePoint, idx: number) => {
