...
import { RefreshCw, ArrowRight, Compass, Search, PenTool, Palette, Eye, Send } from "lucide-react";

const LOOP_STEPS = [
  { step: "DISCOVER", icon: Compass, desc: "Brand vision & target audience audit" },
  { step: "RESEARCH", icon: Search, desc: "Competitive visual landscape analysis" },
  { step: "CONCEPT", icon: PenTool, desc: "Sketches & moodboard exploration" },
  { step: "DESIGN", icon: Palette, desc: "Vector perfection & UI crafting" },
  { step: "REVIEW", icon: Eye, desc: "Feedback iteration & detail refinement" },
  { step: "EXPORT", icon: Send, desc: "Full vector, print & web asset delivery" },
];

export function GraphicDesignOptimizationLoop() {
  return (
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-6 sm:p-10 shadow-xl overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-ink/8 pb-5 mb-8">
        ...
        <div className="flex items-center gap-2 text-xs font-mono font-bold text-blue-900 bg-blue-100/80 px-3.5 py-1.5 rounded-full border border-blue-200">
          <span>ONE-OFF GRAPHICS</span>
          <span className="text-blue-600 font-black">|</span>
          <span>CONTINUOUS BRAND COMPOUNDING</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-6 gap-3 relative">
        {LOOP_STEPS.map((item, idx) => {
          ...
          return (
            ...
            >
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 via-indigo-600 to-blue-800 text-white font-bold shadow-md mb-3">
      ...
      <div className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-ink/70 font-mono bg-surface p-3 rounded-xl border border-ink/5">
        ...
        <span className="text-blue-900">FEEDS BRAND CONSISTENCY BACK TO ALL CAMPAIGNS</span>
