...
import { webDevServices } from "@/data/webDevelopment";
...
const [core, ...satellites] = webDevServices;
const STEP = 360 / satellites.length;
...
const DECOR_TAGS: Record<number, string> = { 0: "LIVE", 6: "96.8%" };
...
export function WebDevServicesGrid() {
  ...
  return (
    <section className="relative overflow-hidden bg-transparent py-14 sm:py-20" aria-labelledby="webdev-services-heading">
      <Container>
        <div className="max-w-2xl">
          ...
          <Reveal delay={0.05}>
            <h2 id="webdev-services-heading" className="mt-3 text-3xl font-black tracking-tight text-ink sm:text-4xl">
              Our Website Development Services
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-3 text-sm sm:text-base leading-relaxed text-muted">
              Complete web solutions designed to build, improve, and scale your digital presence.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mt-14 hidden lg:block">
          <Reveal delay={0.1}>
            <div className="relative mx-auto aspect-square w-full max-w-[860px] rounded-[2rem] border border-ink/8 bg-white">
              {/* Fine technical grid */}
              <div
                  ...
                  return (
                    <g key={node.service.title}>
                      <motion.path
                        ...
                        strokeWidth={isHighlighted ? 0.5 : 0.32}
                        strokeLinecap="round"
                      ...
                      {!prefersReducedMotion && (
                        <motion.circle
                          ...
                          transition={{ duration: 2.6, delay: i * 0.25, repeat: Infinity, ease: "linear" }}
                        />
                      )}
                    </g>
                  );
              ...
              {/* Core node — Custom Website Development */}
              <div className="absolute z-10 -translate-x-1/2 -translate-y-1/2" style={{ left: "50%", top: "50%" }}>
                <motion.button
                  ...
                  className="focus-ring group relative flex h-36 w-36 flex-col items-center justify-center gap-1.5 rounded-full border border-blue-400/30 bg-white text-center shadow-[0_15px_40px_-15px_rgba(28,79,161,0.25)]"
                ...
                >
                  <div className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 via-blue-700 to-blue-900 text-white shadow-md">
                    <CoreIcon className="h-5 w-5" aria-hidden="true" />
                  </div>
                  <span className="text-xs font-black leading-tight text-ink">{core.title}</span>
                  <span id={`${uid}-core-desc`} className="px-4 text-[10px] leading-snug text-muted">
                    {core.description}
                  </span>
                </motion.button>
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <motion.button
                      ...
                      className={
                        "focus-ring group flex w-[142px] flex-col gap-1 rounded-2xl border bg-white p-3 text-left shadow-sm transition-colors duration-300 " +
                    ...
                    >
                      <div
                        className={
                          "flex h-8 w-8 items-center justify-center rounded-xl border transition-transform duration-300 group-hover:rotate-6 " +
                          (node.tone === "blue"
                      ...
                      <span className="mt-1 text-xs font-bold text-ink leading-snug">{node.service.title}</span>
                      <span id={`${uid}-${i}-desc`} className="text-[10px] leading-snug text-muted">
                      ...
                      <AnimatePresence>
                        {isHighlighted && (
                          <motion.span
                            ...
                            className={
                              "mt-1 block overflow-hidden text-[9px] font-black uppercase tracking-wider " +
        ...
        <div className="mt-12 lg:hidden">
          <div className="relative rounded-2xl border border-ink/8 bg-white p-5">
            <div className="mb-4 flex items-center justify-between font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-ink/40">
              <span>Web Development System</span>
