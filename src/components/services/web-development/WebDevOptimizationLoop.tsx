...
import { RefreshCw, ArrowRight, Code2, Zap, ShieldCheck, TrendingUp, Cpu } from "lucide-react";

const LOOP_STEPS = [
  { step: "ARCHITECT", icon: Code2, desc: "Design full-stack Next.js component system" },
  { step: "BENCHMARK", icon: Zap, desc: "Audit Core Web Vitals & LCP metrics" },
  { step: "HARDEN", icon: ShieldCheck, desc: "Enforce TypeScript & security protocols" },
  { step: "DEPLOY", icon: Cpu, desc: "Ship to global edge CDN infrastructure" },
  { step: "CONVERT", icon: TrendingUp, desc: "Capture high-yield visitor actions" },
];

export function WebDevOptimizationLoop() {
  return (
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-6 sm:p-10 shadow-xl overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-ink/8 pb-5 mb-8">
        ...
        <div className="flex items-center gap-2 text-xs font-mono font-bold text-blue-900 bg-blue-100/80 px-3.5 py-1.5 rounded-full border border-blue-200">
          ...
          <span>HIGH-CONVERTING DIGITAL PRODUCT</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 relative">
        {LOOP_STEPS.map((item, idx) => {
          ...
          return (
            ...
            >
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-700 to-indigo-900 text-gold-400 shadow-md mb-3">
      ...
      <div className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-ink/70 font-mono bg-surface p-3 rounded-xl border border-ink/5">
        ...
        <span className="text-blue-800">FEEDS CONVERSION DATA BACK TO UX ARCHITECTURE</span>
