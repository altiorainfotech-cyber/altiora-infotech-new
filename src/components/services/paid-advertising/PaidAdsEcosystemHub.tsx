...
import { Cpu, Zap, ArrowRight, Layers, Play, Pause } from "lucide-react";

export function PaidAdsEcosystemHub() {
  const [activeIdx, setActiveIdx] = useState<number>(0);
  const [isPaused, setIsPaused] = useState<boolean>(false);
  const items = paidAdsOverview.coverage;

  const activeItem = items[activeIdx];
  const ActiveIcon = activeItem.icon;

  // Autoplay functionality: cycle active platform node every 2.8 seconds
  useEffect(() => {
    if (isPaused) return;
  ...
  return (
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/50 p-6 sm:p-8 shadow-xl overflow-hidden">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-ink/8 pb-5">
        <div>
          ...
          <h3 className="text-xl sm:text-2xl font-black text-ink mt-1">Interactive Advertising Ecosystem</h3>
        </div>
        <div className="flex items-center gap-2">
          ...
          >
            {isPaused ? <Play className="h-3 w-3" /> : <Pause className="h-3 w-3 text-gold-600 animate-pulse" />}
          ...
          <span className="inline-flex items-center gap-1.5 text-xs font-extrabold text-blue-900 bg-blue-100/80 px-3 py-1 rounded-full border border-blue-200">
            ...
            <span>Single Campaign Engine</span>
          </span>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        {/* Interactive Radial Ecosystem SVG Visualization with 360 Deg Continuous Rotation */}
        <div className="lg:col-span-7 relative flex items-center justify-center aspect-square max-w-[420px] mx-auto w-full">
          <svg className="w-full h-full overflow-visible" viewBox="0 0 400 400">
            ...
            {/* Central Stationary CAMPAIGN ENGINE Node */}
            <g transform="translate(200, 200)" className="z-20">
              <circle r="46" fill="#1c4fa1" className="shadow-2xl" />
            ...
            {/* Rotating Outer Ring & 8 Satellite Platform Nodes (360 Deg Continuous Rotation) */}
            <motion.g
            ...
            >
              {/* Orbit Ring */}
              <circle cx="200" cy="200" r="140" fill="none" stroke="rgba(28, 79, 161, 0.18)" strokeWidth="2" strokeDasharray="6 6" />

              {/* Platform Satellite Nodes & Paths */}
              {items.map((item, idx) => {
                ...
                return (
                  <g key={item.label} className="cursor-pointer" onClick={() => { setActiveIdx(idx); setIsPaused(true); }}>
                    ...
                    <line
                      ...
                      x2={nx}
                      y2={ny}
                      stroke={isActive ? "url(#pathGlow)" : "rgba(20, 21, 26, 0.12)"}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-700 to-blue-900 text-gold-400 shadow-md">
                  <ActiveIcon className="h-6 w-6" />
                </div>
                <div>
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-gold-600">Active Node</span>
              ...
              <div className="mt-5 flex items-center justify-between border-t border-ink/8 pt-4">
                <span className="text-xs font-bold text-blue-900 flex items-center gap-1.5">
                  ...
                  Connected to Campaign Engine
                </span>
                <span className="text-xs font-mono font-bold text-ink/60">Node {activeIdx + 1}/8</span>
              </div>
            </motion.div>
          </AnimatePresence>
