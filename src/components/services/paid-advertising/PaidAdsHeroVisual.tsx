...
export default function PaidAdsHeroVisual() {
  ...
  useEffect(() => {
    ...
    // Orbiting Data Nodes (Audience, Impressions, Clicks, Conversions, Scale)
    const nodeTypes = [
      { name: "Audience", color: 0x3b82f6, radius: 2.8, speed: 0.5, size: 0.22 },
      { name: "Impressions", color: 0x60a5fa, radius: 3.4, speed: -0.4, size: 0.18 },
      { name: "Clicks", color: 0xd3ac3c, radius: 2.2, speed: 0.7, size: 0.24 },
      { name: "Conversions", color: 0x10b981, radius: 3.8, speed: -0.3, size: 0.25 },
      { name: "Scale", color: 0xf59e0b, radius: 4.4, speed: 0.25, size: 0.28 },
    ];
    ...
    // Data Flow Particles (Inward flow towards GROWTH central core)
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const col = colorPick > 0.6 ? cGold : colorPick > 0.3 ? cBlue : cEmerald;
    ...
    // Mouse Parallax & Scroll reaction setup
    let mouseX = 0;
    let mouseY = 0;
    let targetMouseX = 0;
    let targetMouseY = 0;
    ...
    const animate = () => {
      ...
      if (!prefersReducedMotion) {
        ...
        for (let i = 0; i < particleCount; i++) {
          ...
          // Pull vector towards central origin (0,0,0)
          const dist = Math.sqrt(px * px + py * py + pz * pz);
          if (dist < 0.8) {
            // Reset to initial boundary
