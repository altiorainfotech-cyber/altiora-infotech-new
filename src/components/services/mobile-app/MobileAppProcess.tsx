...
import { mobileAppProcess } from "@/data/mobileApp";
...
export function MobileAppProcess() {
  ...
  return (
    <section ref={containerRef} className="relative overflow-hidden bg-transparent py-14 sm:py-24" aria-labelledby="mobile-app-process-heading">
      ...
      <Container>
        <div className="max-w-2xl">
          ...
          <Reveal delay={0.05}>
            <h2 id="mobile-app-process-heading" className="mt-3 text-3xl font-black tracking-tight text-ink sm:text-5xl leading-tight">
              Continuous Mobile App Lifecycle
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-3 text-sm sm:text-base leading-relaxed text-muted font-medium">
              Watch your mobile app build in real time. Scroll progressively constructs each engineering stage and loops user feedback back into product strategy.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mt-14 rounded-3xl border border-ink/10 bg-gradient-to-br from-slate-900 via-blue-950 to-ink p-6 sm:p-12 text-white shadow-2xl relative overflow-hidden">
          <div className="relative mb-8 hidden lg:block">
            ...
            <motion.div
              className="absolute top-0 left-0 h-1.5 rounded-full bg-gradient-to-r from-blue-500 via-gold-400 to-emerald-400 shadow-[0_0_15px_rgba(37,99,235,0.6)]"
              style={{ width: useTransform(progress, [0, 0.8], ["0%", "100%"]) }}
            />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
            {mobileAppProcess.map((step, idx) => {
              const Icon = step.icon;
              const threshold = (idx / (mobileAppProcess.length - 1)) * 0.8;
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-between border-t border-white/10 pt-6 text-xs font-mono font-bold text-white/70 gap-4">
            <div className="flex items-center gap-2">
              ...
              <span>STAGE 06 (SCALE & MAINTAIN) LOOPS BACK TO STAGE 01 (STRATEGY & DISCOVERY)</span>
            </div>
            <div className="flex items-center gap-2 text-gold-400 bg-gold-400/10 px-3 py-1.5 rounded-full border border-gold-400/30">
              ...
              <span>INFINITE APP FLYWHEEL</span>
            </div>
          </div>
        </div>
      </Container>
