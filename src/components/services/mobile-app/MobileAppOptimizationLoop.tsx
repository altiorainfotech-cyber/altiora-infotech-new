...
import { RefreshCw, ArrowRight, Smartphone, Code2, ShieldCheck, TrendingUp, Cpu } from "lucide-react";

const LOOP_STEPS = [
  { step: "CONCEPT", icon: Smartphone, desc: "Wireframe mobile UX & user journey" },
  { step: "ENGINEER", icon: Code2, desc: "Build iOS, Android & cross-platform code" },
  { step: "HARDEN", icon: ShieldCheck, desc: "Automate QA & crash telemetry testing" },
  { step: "PUBLISH", icon: Cpu, desc: "Deploy to App Store & Google Play" },
  { step: "ENGAGE", icon: TrendingUp, desc: "Drive user retention & in-app conversions" },
];

export function MobileAppOptimizationLoop() {
  return (
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-6 sm:p-10 shadow-xl overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-ink/8 pb-5 mb-8">
        ...
        <div className="flex items-center gap-2 text-xs font-mono font-bold text-blue-900 bg-blue-100/80 px-3.5 py-1.5 rounded-full border border-blue-200">
          ...
          <span>HIGH-RETENTION MOBILE ENGINE</span>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-ink/70 font-mono bg-surface p-3 rounded-xl border border-ink/5">
        ...
        <span className="text-blue-800">FEEDS USER TELEMETRY BACK TO PRODUCT FEATURE MAP</span>
