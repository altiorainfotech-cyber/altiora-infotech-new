...
import { mobileAppServices } from "@/data/mobileApp";
import { Sparkles, Cpu, Play, Pause } from "lucide-react";
import { MobileAppOptimizationLoop } from "./MobileAppOptimizationLoop";

export function MobileAppServicesGrid() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveNode((prev) => (prev + 1) % mobileAppServices.length);
    }, 3000);
  ...
  return (
    <section ref={containerRef} className="relative overflow-hidden bg-transparent py-14 sm:py-24" aria-labelledby="mobile-app-services-heading">
      <Container>
        <div className="max-w-2xl">
          ...
          <Reveal delay={0.05}>
            <h2 id="mobile-app-services-heading" className="mt-3 text-3xl font-black tracking-tight text-ink sm:text-5xl leading-tight">
              Mobile App Intelligence System
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-3 text-sm sm:text-base leading-relaxed text-muted font-medium">
              Six connected mobile disciplines forming one continuous digital app engine — where user analytics and crash telemetry feed back into UX.
            </p>
          </Reveal>
        </div>
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {mobileAppServices.map((serv, idx) => (
                  <button
                    key={serv.title}
