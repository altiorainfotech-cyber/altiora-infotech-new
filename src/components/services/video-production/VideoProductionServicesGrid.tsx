...
import { videoProductionServices, VideoService } from "@/data/videoProduction";
import { Sparkles, Cpu, Play, Pause } from "lucide-react";
import { VideoProductionOptimizationLoop } from "./VideoProductionOptimizationLoop";

export function VideoProductionServicesGrid() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveNode((prev) => (prev + 1) % videoProductionServices.length);
    }, 3000);
  ...
  return (
    <section ref={containerRef} className="relative overflow-hidden bg-transparent py-14 sm:py-24" aria-labelledby="video-services-heading">
      <Container>
        <div className="max-w-2xl">
          ...
          <Reveal delay={0.05}>
            <h2 id="video-services-heading" className="mt-3 text-3xl font-black tracking-tight text-ink sm:text-5xl leading-tight">
              Video Intelligence System
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-3 text-sm sm:text-base leading-relaxed text-muted font-medium">
              Six connected video production disciplines working in a continuous narrative loop.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mt-12 rounded-3xl border border-ink/10 bg-gradient-to-br from-slate-900 via-blue-950 to-ink p-6 sm:p-12 text-white shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4 max-w-xl mx-auto mb-10 text-center sm:text-left">
            <div>
              <span className="text-xs font-mono font-bold uppercase tracking-widest text-blue-400">Interactive Video Engine</span>
              <h3 className="text-xl sm:text-3xl font-black text-white mt-1">Continuous Production Optimization</h3>
            </div>
            <button
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-7 relative flex items-center justify-center aspect-square max-w-[460px] mx-auto w-full">
              <svg className="w-full h-full overflow-visible" viewBox="0 0 440 440">
                <defs>
                  <linearGradient id="videoLoopGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                ...
                >
                  ...
                  {videoProductionServices.map((service: VideoService, i: number) => {
                    const angle = (i / videoProductionServices.length) * Math.PI * 2 - Math.PI / 2;
                    const radius = 150;
                    ...
                    return (
                      <g key={service.title} className="cursor-pointer" onClick={() => { setActiveNode(i); setIsPaused(true); }}>
                        <line
                          ...
                          x2={nx}
                          y2={ny}
                          stroke={isActive ? "url(#videoLoopGrad)" : "rgba(255, 255, 255, 0.2)"}
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="rounded-2xl border border-white/10 bg-white/10 p-6 backdrop-blur-md">
                ...
                <h4 className="text-xl font-black text-white mt-1">
                  {videoProductionServices[activeNode].title}
                </h4>
                <p className="mt-2 text-sm leading-relaxed text-white/80 font-medium">
                  {videoProductionServices[activeNode].description}
                </p>
                <div className="mt-4 inline-flex items-center gap-2 rounded-full bg-blue-400/20 border border-blue-400/40 px-3 py-1 text-xs font-bold text-blue-300">
                  ...
                  <span>{videoProductionServices[activeNode].metric}</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2">
                {videoProductionServices.map((serv: VideoService, idx: number) => (
                  <button
                    key={serv.title}
        ...
        <div className="mt-16">
          <VideoProductionOptimizationLoop />
        </div>
      </Container>
    </section>
  );
