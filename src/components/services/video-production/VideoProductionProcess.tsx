...
import { videoProductionProcess } from "@/data/videoProduction";
...
export function VideoProductionProcess() {
  ...
  return (
    <section className="relative overflow-hidden bg-transparent py-14 sm:py-20" aria-labelledby="video-process-heading">
      ...
      <Container>
        <div className="max-w-2xl">
          <Reveal>
            <div className="inline-flex items-center gap-2 rounded-full border border-blue-400/30 bg-blue-50/80 px-3.5 py-1 text-xs font-extrabold text-blue-800">
              ...
              <span>How We Bring Your Story to Life</span>
            </div>
          </Reveal>
          <Reveal delay={0.05}>
            <h2 id="video-process-heading" className="mt-3 text-3xl font-black tracking-tight text-ink sm:text-4xl">
        ...
        <ol ref={trackRef} className="relative mt-16 grid grid-cols-2 gap-x-6 gap-y-12 sm:grid-cols-3 lg:grid-cols-6">
          ...
          {videoProductionProcess.map((step, idx) => {
