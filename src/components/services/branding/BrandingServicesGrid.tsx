...
import { brandingServices, brandingServicesIntro } from "@/data/branding";
...
const [core, ...satellites] = brandingServices;
...
const DECOR_TAGS: Record<number, string> = { 1: "LIVE", 4: "AUDIT" };
...
export function BrandingServicesGrid() {
  ...
  return (
    <section className="relative overflow-hidden bg-transparent py-14 sm:py-20" aria-labelledby="branding-services-heading">
      <Container>
        <div className="max-w-2xl">
          <Reveal>
            <div className="inline-flex items-center gap-2 rounded-full border border-gold-400/30 bg-gold-50/80 px-3.5 py-1 text-xs font-extrabold text-gold-800">
              ...
              <span>{brandingServicesIntro.eyebrow}</span>
            </div>
          </Reveal>
          <Reveal delay={0.05}>
            <h2 id="branding-services-heading" className="mt-3 text-3xl font-black tracking-tight text-ink sm:text-4xl">
              {brandingServicesIntro.heading}
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-3 text-sm sm:text-base leading-relaxed text-muted">
              {brandingServicesIntro.description}
            </p>
          </Reveal>
        </div>
