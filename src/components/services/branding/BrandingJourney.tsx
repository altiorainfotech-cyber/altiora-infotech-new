...
import { SearchCheck, Compass, Palette, MessageCircle, BookOpen, Rocket } from "lucide-react";

const STAGES = [
  { step: "01", title: "AUDIT", icon: SearchCheck, desc: "Market research & brand perception audit" },
  { step: "02", title: "STRATEGY", icon: Compass, desc: "Positioning, vision & value proposition" },
  { step: "03", title: "IDENTITY", icon: Palette, desc: "Logo, typography & color system design" },
  { step: "04", title: "VOICE", icon: MessageCircle, desc: "Brand copy, taglines & tone of voice" },
  { step: "05", title: "SYSTEM", icon: BookOpen, desc: "Comprehensive brand guidelines documentation" },
  { step: "06", title: "LAUNCH", icon: Rocket, desc: "Omnichannel brand rollout & team enablement" },
];

export function BrandingJourney() {
  return (
    <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-blue-950 to-ink p-6 sm:p-10 text-white shadow-2xl relative overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-5 mb-8">
        <div>
          <span className="text-xs font-mono font-bold uppercase tracking-wider text-gold-400">Brand Pipeline</span>
          <h3 className="text-xl sm:text-2xl font-black text-white mt-1">Brand Lifecycle Architecture</h3>
        </div>
        <div className="flex items-center gap-2 text-xs font-mono font-bold text-gold-400 bg-white/5 px-3.5 py-1.5 rounded-full border border-white/10">
          <span>6-STAGE BRAND PIPELINE</span>
        </div>
      </div>
