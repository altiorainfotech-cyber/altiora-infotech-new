...
import { RefreshCw, ArrowRight, SearchCheck, Compass, Palette, MessageCircle, BookOpen, Rocket } from "lucide-react";

const LOOP_STEPS = [
  { step: "AUDIT", icon: SearchCheck, desc: "Crawl market & perception gaps" },
  { step: "POSITION", icon: Compass, desc: "Define unique market space" },
  { step: "DESIGN", icon: Palette, desc: "Craft logo, colors & typography" },
  { step: "VOICE", icon: MessageCircle, desc: "Write taglines & messaging rules" },
  { step: "SYSTEM", icon: BookOpen, desc: "Document full brand guidelines" },
  { step: "LAUNCH", icon: Rocket, desc: "Roll out & measure brand equity" },
];

export function BrandingOptimizationLoop() {
  return (
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-6 sm:p-10 shadow-xl overflow-hidden">
      ...
      <div className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-ink/70 font-mono bg-surface p-3 rounded-xl border border-ink/5">
        ...
        <span className="text-blue-900">FEEDS BRAND RECOGNITION BACK TO AUTHORITY &amp; PRICING</span>
