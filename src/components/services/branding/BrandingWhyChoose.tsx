...
import { whyChooseBranding, WhyChoosePoint } from "@/data/branding";
import { ShieldCheck } from "lucide-react";

export function BrandingWhyChoose() {
  return (
    <section className="relative overflow-hidden bg-transparent py-14 sm:py-24" aria-labelledby="branding-why-heading">
      <Container>
        <div className="max-w-2xl">
          ...
          <Reveal delay={0.05}>
            <h2 id="branding-why-heading" className="mt-3 text-3xl font-black tracking-tight text-ink sm:text-5xl leading-tight">
              Why Brands Choose Altiora for Brand Strategy
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-3 text-sm sm:text-base leading-relaxed text-muted font-medium">
              We build brands that don't just look impressive — they actively command premium pricing and market trust.
            </p>
          </Reveal>
        </div>

        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {whyChooseBranding.map((item: WhyChoosePoint, idx: number) => {
