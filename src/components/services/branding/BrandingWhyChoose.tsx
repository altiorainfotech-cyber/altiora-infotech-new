...
import { whyChooseBranding, whyChooseBrandingIntro } from "@/data/branding";
import { ShieldCheck } from "lucide-react";

export function BrandingWhyChoose() {
  return (
    <section className="relative overflow-hidden bg-transparent py-14 sm:py-20" aria-labelledby="branding-why-choose-heading">
        ...
        <div className="max-w-2xl">
          <Reveal>
            <div className="inline-flex items-center gap-2 rounded-full border border-blue-400/30 bg-blue-50/80 px-3.5 py-1 text-xs font-extrabold text-blue-800">
              ...
              <span>{whyChooseBrandingIntro.eyebrow}</span>
            </div>
          </Reveal>
          <Reveal delay={0.05}>
            <h2 id="branding-why-choose-heading" className="mt-3 text-3xl font-black tracking-tight text-ink sm:text-4xl">
              {whyChooseBrandingIntro.heading}
            </h2>
          </Reveal>
          <Reveal delay={0.1}>
            <p className="mt-3 text-sm sm:text-base leading-relaxed text-muted">
              {whyChooseBrandingIntro.description}
            </p>
          </Reveal>
        </div>

        <div className="mt-10 grid grid-cols-1 gap-x-12 gap-y-8 sm:grid-cols-2">
          {whyChooseBranding.map((point, idx) => {
