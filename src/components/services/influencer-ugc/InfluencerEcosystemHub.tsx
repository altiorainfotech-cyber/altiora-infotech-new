...
import { influencerOverview, CreatorType } from "@/data/influencerUgc";
import { Cpu, Zap, Layers, Play, Pause } from "lucide-react";

export function InfluencerEcosystemHub() {
  const [activeIdx, setActiveIdx] = useState<number>(0);
  const [isPaused, setIsPaused] = useState<boolean>(false);
  const items = influencerOverview.coverage || influencerOverview.capabilities;
    ...
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-6 sm:p-8 shadow-xl overflow-hidden">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-ink/8 pb-5">
        <div>
          ...
          <h3 className="text-xl sm:text-2xl font-black text-ink mt-1">Interactive Creator Ecosystem</h3>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        <div className="lg:col-span-7 relative flex items-center justify-center aspect-square max-w-[420px] mx-auto w-full">
          <svg className="w-full h-full overflow-visible" viewBox="0 0 400 400">
            <defs>
              <linearGradient id="influencerPathGlow" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            >
              <circle cx="200" cy="200" r="140" fill="none" stroke="rgba(28, 79, 161, 0.2)" strokeWidth="2" strokeDasharray="6 6" />

              {items.map((item: CreatorType, idx: number) => {
                ...
                return (
                  <g key={item.label} className="cursor-pointer" onClick={() => { setActiveIdx(idx); setIsPaused(true); }}>
                    <line
                      ...
                      x2={nx}
                      y2={ny}
                      stroke={isActive ? "url(#influencerPathGlow)" : "rgba(20, 21, 26, 0.12)"}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-blue-900 text-gold-400 shadow-md">
                  <ActiveIcon className="h-6 w-6" />
                </div>
                <div>
                  <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-blue-600">Active Creator Node</span>
