...
import { RefreshCw, ArrowRight, Users, Camera, ShieldCheck, Share2, LineChart, TrendingUp } from "lucide-react";

const LOOP_STEPS = [
  { step: "MATCH", icon: Users, desc: "AI-driven creator vetting & alignment" },
  { step: "BRIEF", icon: ShieldCheck, desc: "Brand compliance & creative guidelines" },
  { step: "PRODUCE", icon: Camera, desc: "Authentic UGC video & asset creation" },
  { step: "AMPLIFY", icon: Share2, desc: "Cross-platform ad whitelisting" },
  { step: "ANALYZE", icon: LineChart, desc: "Track conversions, CPL & ROAS" },
  { step: "SCALE", icon: TrendingUp, desc: "Double down on top-performing creators" },
];

export function InfluencerOptimizationLoop() {
  return (
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-6 sm:p-10 shadow-xl overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-ink/8 pb-5 mb-8">
        ...
        <div className="flex items-center gap-2 text-xs font-mono font-bold text-blue-900 bg-blue-100/80 px-3.5 py-1.5 rounded-full border border-blue-200">
          <span>ONE-OFF SHOUTOUTS</span>
          <span className="text-gold-600 font-black">|</span>
          <span>CONTINUOUS CREATOR SCALING</span>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-ink/70 font-mono bg-surface p-3 rounded-xl border border-ink/5">
        ...
        <span className="text-blue-800">FEEDS REVENUE BACK TO EXPAND CREATOR ROSTER</span>
