...
import { businessConsultingOverview, ConsultingCapability } from "@/data/businessConsulting";
import { Cpu, Zap, Layers, Play, Pause } from "lucide-react";

export function BusinessConsultingEcosystemHub() {
  const [activeIdx, setActiveIdx] = useState<number>(0);
  const [isPaused, setIsPaused] = useState<boolean>(false);
  const items = businessConsultingOverview.coverage || businessConsultingOverview.capabilities;
    ...
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-6 sm:p-8 shadow-xl overflow-hidden">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-ink/8 pb-5">
        <div>
          ...
          <h3 className="text-xl sm:text-2xl font-black text-ink mt-1">Interactive Consulting Ecosystem</h3>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1.5 text-xs font-extrabold text-blue-900 bg-blue-100/80 px-3 py-1 rounded-full border border-blue-200">
            ...
            <span>Single Advisory Engine</span>
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        <div className="lg:col-span-7 relative flex items-center justify-center aspect-square max-w-[420px] mx-auto w-full">
          <svg className="w-full h-full overflow-visible" viewBox="0 0 400 400">
            <defs>
              <linearGradient id="consultingPathGlow" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <g transform="translate(200, 200)" className="z-20">
              <circle r="46" fill="#1d4ed8" stroke="#d3ac3c" strokeWidth="2.5" className="shadow-2xl" />
            ...
            >
              <circle cx="200" cy="200" r="140" fill="none" stroke="rgba(29, 78, 216, 0.2)" strokeWidth="2" strokeDasharray="6 6" />

              {items.map((item: ConsultingCapability, idx: number) => {
                ...
                return (
                  <g key={item.label} className="cursor-pointer" onClick={() => { setActiveIdx(idx); setIsPaused(true); }}>
                    <line
                      ...
                      x2={nx}
                      y2={ny}
                      stroke={isActive ? "url(#consultingPathGlow)" : "rgba(20, 21, 26, 0.12)"}
                    ...
                    <g transform={`translate(${nx}, ${ny})`}>
                      ...
                      >
                        ...
                        >
                          <NodeIcon className={`h-4 w-4 ${isActive ? "text-ink font-extrabold" : "text-white"}`} />
                        </motion.div>
                      </foreignObject>
                    </g>
                  </g>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 via-indigo-600 to-blue-800 text-white shadow-md font-bold">
