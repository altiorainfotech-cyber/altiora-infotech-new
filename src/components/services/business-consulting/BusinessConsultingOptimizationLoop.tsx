...
import { RefreshCw, ArrowRight, SearchCheck, BarChart3, Compass, Rocket, LineChart, TrendingUp } from "lucide-react";

const LOOP_STEPS = [
  { step: "AUDIT", icon: SearchCheck, desc: "Operational & market discovery" },
  { step: "ANALYZE", icon: BarChart3, desc: "Identify growth & margin gaps" },
  { step: "STRATEGIZE", icon: Compass, desc: "Build targeted growth roadmaps" },
  { step: "EXECUTE", icon: Rocket, desc: "Deploy workflow & team optimization" },
  { step: "MEASURE", icon: LineChart, desc: "Track KPIs & operational ROI" },
  { step: "SCALE", icon: TrendingUp, desc: "Expand market share & revenue" },
];

export function BusinessConsultingOptimizationLoop() {
  return (
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-6 sm:p-10 shadow-xl overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-ink/8 pb-5 mb-8">
        ...
        <div className="flex items-center gap-2 text-xs font-mono font-bold text-blue-900 bg-blue-100/80 px-3.5 py-1.5 rounded-full border border-blue-200">
          <span>ONE-OFF AUDIT</span>
          <span className="text-gold-600 font-black">|</span>
          <span>CONTINUOUS BUSINESS COMPOUNDING</span>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-ink/70 font-mono bg-surface p-3 rounded-xl border border-ink/5">
        ...
        <span className="text-blue-800">FEEDS REVENUE BACK TO STRATEGIC EXPANSION &amp; M&amp;A</span>
