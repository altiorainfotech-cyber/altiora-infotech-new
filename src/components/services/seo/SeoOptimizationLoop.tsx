...
import { RefreshCw, ArrowRight, Database, Search, Cpu, FileText, TrendingUp } from "lucide-react";

const LOOP_STEPS = [
  { step: "CRAWL", icon: Database, desc: "Gather technical & index signals" },
  { step: "ANALYZE", icon: Search, desc: "Identify keyword & competitor gaps" },
  { step: "OPTIMIZE", icon: FileText, desc: "Enhance entities & on-page copy" },
  { step: "RANK", icon: Cpu, desc: "Secure Page 1 & AI Answer positions" },
  { step: "CONVERT", icon: TrendingUp, desc: "Turn organic traffic into leads" },
];

export function SeoOptimizationLoop() {
  return (
    <div className="relative rounded-3xl border border-ink/8 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-6 sm:p-10 shadow-xl overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-ink/8 pb-5 mb-8">
        ...
        <div className="flex items-center gap-2 text-xs font-mono font-bold text-blue-900 bg-blue-100/80 px-3.5 py-1.5 rounded-full border border-blue-200">
          ...
          <span>CONTINUOUS ORGANIC COMPOUNDING</span>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-center gap-2 text-xs font-bold text-ink/70 font-mono bg-surface p-3 rounded-xl border border-ink/5">
        ...
        <span className="text-blue-800">FEEDS REVENUE BACK TO AUTHORITY &amp; CONTENT</span>
