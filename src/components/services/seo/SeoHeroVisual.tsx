...
export default function SeoHeroVisual() {
  ...
  useEffect(() => {
    ...
    // Central "ORGANIC RANK" core node
    ...
    // Orbiting SEO Nodes (Keywords, Technical, Content, Backlinks, AEO)
    const nodeTypes = [
      { name: "Keywords", color: 0x3b82f6, radius: 2.8, speed: 0.5, size: 0.22 },
      { name: "Technical", color: 0x10b981, radius: 3.4, speed: -0.4, size: 0.18 },
      { name: "Content", color: 0xd3ac3c, radius: 2.2, speed: 0.7, size: 0.24 },
      { name: "Backlinks", color: 0x6366f1, radius: 3.8, speed: -0.3, size: 0.25 },
      { name: "AEO/GEO", color: 0xf59e0b, radius: 4.4, speed: 0.25, size: 0.28 },
    ];
    ...
    ringRadii.forEach((r, idx) => {
      ...
      const mat = new THREE.MeshStandardMaterial({
        color: idx % 2 === 0 ? 0x10b981 : 0x2563eb,
        emissive: idx % 2 === 0 ? 0x059669 : 0x1d4ed8,
    ...
    // Organic Traffic Stream Particles
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const col = colorPick > 0.6 ? cEmerald : colorPick > 0.3 ? cBlue : cGold;
